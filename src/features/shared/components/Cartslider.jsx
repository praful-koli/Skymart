import { useCart } from "../../product/hook/useCart";
...
export default function CartSlider() {
  const { items, isOpen, closeCart, removeItem, updateQty, clearCart, total, count ,setIsOpen} = useCart();
      ...
      <div className={`cart-slider ${isOpen ? "cart-slider-open" : ""}`}>
        ...
        <div className="cart-items">
          {items.length === 0 ? (
            <div className="cart-empty w-full">
              ...
              <button onClick={()=> browserProductHandler()} className="  w-35 h-10 border-none rounded-2xl  bg-[#C8F400] text-sm text-black font-normal">Browse Products</button>
            </div>
          ) : (
            items.map((item) => (
              <div className="cart-item" key={item.id}>
                ...
                <div className="cart-item-info">
                  ...
                  <p className="cart-item-each">${item.price.toFixed(2)} each</p>
                </div>
        ...
        {items.length > 0 && (
          <div className="cart-footer">
            ...
            <button className="cart-checkout-btn" onClick={()=> checkOutHander()}>
              Checkout <ArrowIcon />
            </button>
